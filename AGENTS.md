...
7. Changed the defs or canvas settings? `POST /api/render` re-renders everything (motion frames re-render their clips).
8. `GET /api/export/zip?projectId=` → `FNN.png` + `clips/FNN/%04d.png` + `storyboard.json` (timeline `startSec/durationSec`, all SVG + motion sources) + `captions.srt` (same timeline) + `assemble.sh` (**`sh assemble.sh` → film.mp4** via ffmpeg) + `gltf/FNN.gltf`. For path-traced 3D: `POST /api/export/gltf {motion|spec}` → `blender -b -P scripts/blender_render.py -- shot.gltf out/ --engine CYCLES`.
