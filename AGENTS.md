...
- **Director (Nemotron crew, ADR-017)** lives in `src/lib/services/director/` + `src/lib/providers/`. LLM output NEVER bypasses the engine's validators: zod schemas (`director/schemas.ts`) → `sanitizeSvg` → `constructSpecSchema`/`motionSpecSchema`. Writes go through `frameWrites.ts` (the same path the REST routes use). Runs stream over SSE inside the request (no queue); every model call is a `DirectorStep`. Tests use `MockLlmProvider`/`demoHandler` — **no network in tests**. Live-only scripts: `npm run director:{models,smoke,showcase,bench}` (spend ledger, $15 stop line). Secrets only in `.env.local` / platform env — never in tracked files.
